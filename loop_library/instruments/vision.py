...
class VisionInstrument(BaseInstrument):
    ...
    async def execute(
        ...
        while iteration < self.max_iterations:
            ...
            prompt = self._build_analysis_prompt(query, context, iteration, previous_analysis)

            response = await self.claude.complete_with_images(prompt, images, system=system)
            ...
            confidence = self.termination.calculate_confidence(
                findings, 1, has_answer=any(f.confidence > 0.8 for f in new_findings),
            )
            confidence_history.append(confidence)

            result = self.termination.evaluate(
                findings, iteration, self.max_iterations, confidence_history, previous_finding_count,
            )
                ...
                try:
                    await checkpoint_fn(
                        ...
                        {"new_findings": len(new_findings), "total_findings": len(findings),
                         "confidence": confidence, "should_terminate": result.should_terminate},
                        iteration_ms,
                    )
        ...
        return InstrumentResult(
            ...
            suggested_followups=await self._suggest_followups(query, findings, outcome),
        )
        ...
        for attachment in attachments:
            ...
            if match:
                images.append(ImageInput(
                    source_type="base64", media_type=match.group(1), data=match.group(2),
                ))
                continue
            if attachment.startswith("https://"):
                ...
                for ext, mime in _EXT_TO_MEDIA_TYPE.items():
                    if lower.endswith(ext):
                        media_type = mime
                        break
                images.append(ImageInput(source_type="url", media_type=media_type, data=attachment))
                continue
            logger.debug(f"Skipping non-image attachment: {attachment[:50]}...")
        return images

    @staticmethod
    def _build_system_prompt(iteration: int, previous_analysis: str | None) -> str:
        base = (
    ...
    def _build_analysis_prompt(
        query: str, context: TaskContext | None, iteration: int, previous_analysis: str | None,
    ) -> str:
    ...
    def _extract_findings(self, response: str, iteration: int) -> list[Finding]:
            ...
            for obs in observations:
                if isinstance(obs, str) and obs.strip():
                    findings.append(Finding(
                        content=obs, source="claude_vision", confidence=response_confidence,
                    ))
            if findings:
                return findings
        return [Finding(content=response[:1000], source="claude_vision", confidence=0.7)]

    async def _synthesize_analysis(self, query: str, findings: list[Finding]) -> str:
        if not findings:
